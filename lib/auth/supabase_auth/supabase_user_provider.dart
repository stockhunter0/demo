...
class StockHunterSupabaseUser extends BaseAuthUser {
  StockHunterSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> stockHunterSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = StockHunterSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
